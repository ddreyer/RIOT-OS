...
static void clk_init(void)
{
    ...
    SYSCTRL->DFLLMUL.reg = SYSCTRL_DFLLMUL_CSTEP(coarse >> 1) |
                           SYSCTRL_DFLLMUL_FSTEP(fine >> 1) |
                           SYSCTRL_DFLLMUL_MUL(CLOCK_CORECLOCK_HIGH / CLOCK_OSCULP32K);
    SYSCTRL->DFLLVAL.reg = SYSCTRL_DFLLVAL_COARSE(coarse) |
                           SYSCTRL_DFLLVAL_FINE(fine);
    SYSCTRL->DFLLCTRL.reg = SYSCTRL_DFLLCTRL_MODE;
    while ((SYSCTRL->PCLKSR.reg & SYSCTRL_PCLKSR_DFLLRDY) == 0) {
...
static bool current_fastclk = false;

bool get_sysclk(void) {
    return current_fastclk;
}

void sysclk_change(bool fastclk) {
    unsigned irq_state = irq_disable();
    if (fastclk && !current_fastclk) {
        PM->APBBMASK.reg |= PM_APBBMASK_NVMCTRL;
        NVMCTRL->CTRLB.reg |= NVMCTRL_CTRLB_RWS((CLOCK_CORECLOCK_HIGH - 1) / 24000000);
        PM->APBBMASK.reg &= ~PM_APBBMASK_NVMCTRL;
        ...
        GCLK->GENCTRL.reg = (GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_DFLL48M | GCLK_GENCTRL_ID(0));
        while (GCLK->STATUS.reg & GCLK_STATUS_SYNCBUSY) {}
        current_fastclk = true;
    } else if (!fastclk && current_fastclk) {
        GCLK->GENCTRL.reg = (GCLK_GENCTRL_GENEN | GCLK_GENCTRL_SRC_OSC8M | GCLK_GENCTRL_ID(0));
        while (GCLK->STATUS.reg & GCLK_STATUS_SYNCBUSY) {}

        PM->APBBMASK.reg |= PM_APBBMASK_NVMCTRL;
        NVMCTRL->CTRLB.reg |= NVMCTRL_CTRLB_RWS((CLOCK_CORECLOCK - 1) / 24000000);
        PM->APBBMASK.reg &= ~PM_APBBMASK_NVMCTRL;

        SYSCTRL->DFLLCTRL.bit.ENABLE = 0;
        current_fastclk = false;
    }
